...
import "./libraries/SyntheticHelper.sol";
import "./libraries/TickMath.sol";
import "./libraries/LiquidityMath.sol";
import "./libraries/SqrtPriceMath.sol";
import "./libraries/IgniteMath.sol";
...
contract RadbotV1Deployer is IRadbotV1Deployer, NoDelegateCall {
    ...
    /// @dev Prevents calling a function from anyone except the address returned by IRadbotV1Factory#owner()
    modifier onlyFactoryOwner() {
        require(msg.sender == IRadbotV1Factory(factory).owner(), "O");
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = IRadbotV1Launcher(
            msg.sender
        ).parameters();
    ...
    function balance0() private view returns (uint256) {
        (bool success, bytes memory data) = token0.staticcall(
            abi.encodeWithSelector(
                IRadbotSynthetic.balance.selector,
                address(this)
            )
        );
    ...
    /// @inheritdoc IRadbotV1DeployerDerivedState
    ...
    function mint(
        ...
        // Mint tokens BEFORE the callback
        if (amount0 > 0)
            SyntheticHelper.safeMint(token0, address(this), amount0);
        if (amount1 > 0)
            SyntheticHelper.safeMint(token1, address(this), amount1);

        IRadbotV1MintCallback(msg.sender).radbotV1MintCallback(
            amount0,
            amount1,
            data
        );
    ...
    /// @inheritdoc IRadbotV1DeployerOwnerActions
        ...
        returns (int256 amount0, int256 amount1)
    {
        ...
        if (zeroForOne) {
            ...
            IRadbotV1IgniteCallback(msg.sender).radbotV1IgniteCallback(
                amount0,
                amount1,
                data
            );
            ...
            SyntheticHelper.safeMint(token1, address(this), uint256(amount1));

            require(balance1() >= balance1Before.add(uint256(amount1)), "IIA");
        }
